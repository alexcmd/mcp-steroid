...
class WirePristinenessTest {
    ...
    @Test
    fun `MCP-only ListedWindow DOES serialize backend_name`() {
        ...
        // The mapper copies every wire field verbatim.
        assertEquals("x", decoded.projectName)
        assertEquals("/p", decoded.projectPath)
    ...
    @Test
    fun `MCP-only ListedBackgroundTask DOES serialize backend_name`() {
