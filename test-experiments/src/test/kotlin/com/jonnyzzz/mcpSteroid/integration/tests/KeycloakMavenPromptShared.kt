...
/**
 * Shared task prompt for [KeycloakMavenTest].
 *
 * Same shape as [buildYouTrackDbMavenPrompt]: the agent must drive Maven through
 * IntelliJ's `MavenRunConfigurationType` (NOT `Bash ./mvnw`), capture pass/fail
 * via the run-content descriptor's `ProcessHandler`, and report the result through
 * the marker contract below.
 *
 * Keycloak's `core` Maven module holds plain JUnit unit tests (e.g.
 * `org.keycloak.util.JsonParserTest`, `org.keycloak.crypto.HashTest`) that compile
 * and run in tens of seconds. Tests under `services`, `integration`, `quarkus`,
 * or anything named `*IT`/`*ITest` boot a server and must be avoided.
 */
internal fun buildKeycloakMavenPrompt(): String = buildString {
    appendLine("The Keycloak Java project (https://github.com/keycloak/keycloak) is open in IntelliJ IDEA. It is a multi-module Apache Maven project.")
    appendLine()
    ...
    appendLine("Avoid integration tests, anything named `*IT` / `*ITest`, anything using `@Testcontainers`, anything that boots a server (e.g. tests under `services`, `integration`, `quarkus`, `testsuite`), and anything that touches the network or large disk fixtures.")
    ...
    appendLine("- If the first run fails because a sibling Maven module is missing from `~/.m2` (e.g. `keycloak-common`), use the *Sibling-install fallback* from the recipe: install just that one module via the same `createRunnerAndConfigurationSettings` shape with goals `install -pl <missing-module> -DskipTests`. Then retry the targeted test.")
    ...
    appendLine("KEYCLOAK_MAVEN_TEST_RAN: yes")
}
 ...
 *
 * Mirrors [assertYouTrackDbMavenAgentSucceeded] one-for-one — only the
 * project-specific marker name differs.
 */
internal fun assertKeycloakMavenAgentSucceeded(combined: String) {
    check(combined.contains("KEYCLOAK_MAVEN_TEST_RAN: yes", ignoreCase = false)) {
        "Agent did not report KEYCLOAK_MAVEN_TEST_RAN: yes.\nOutput:\n$combined"
    }
    ...
    check(combined.contains("TEST_RESULT: PASSED")) {
        "Agent did not report TEST_RESULT: PASSED — the chosen test must actually pass " +
            "(captured via processHandler.exitCode == 0 after termination).\nOutput:\n$combined"
    }
